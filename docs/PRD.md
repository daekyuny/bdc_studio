...
| Feature | Description | Status |
|---|---|---|
| Burndown chart | SVG ideal (blue), actual (red), and scope (green dashed) line chart with N+1 border model; ideal reaches exactly 0; ideal line starting point raised to account for tasks moved in from other sprints before sprint start; Today shown as shaded band (current sprint only); browse marker (past/future sprints); click x-axis date label to set TODAY | Done |
...
| 2026-03-20 | 1.4 | Burndown chart: ideal line now correctly raised when tasks are moved in from other sprints before sprint start. CSS border visibility improvements. |
