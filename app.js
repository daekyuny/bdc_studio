...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  var saveTeamState = async (teamId, state2) => {
    await setDoc(doc(db, "appdata", teamId), JSON.parse(JSON.stringify(state2)));
  };
  var subscribeToTeamState = (teamId, callback) => {
  ...
  var setUserRole = async (userId, role) => {
  ...
  var doSaveToFirestore = async (teamId) => {
    try {
  ...
  var drawChart = ({ dates, totalPoints, ideal, actual, scope, scopeDropMarkers, todayIndex }, onDateClick, browseIndex, showTodayLabel) => {
    ...
    const nonNullActual = actual.filter((v2) => v2 !== null);
    const nonNullScope = scope.filter((v2) => v2 !== null);
...
${marker.label}`;
    ...
    const actualPoints = actual.map((val, i) => val !== null ? toPoint(val, i) : null).filter((v2) => v2 !== null);
  ...
  var renderStats = (sprint, burndown) => {
    ...
    const lastActual = [...burndown.actual].reverse().find((v2) => v2 !== null) ?? 0;
    ...
    const fmt = (v2) => v2.toFixed(2).replace(/0$/, "");
...
They will be removed from all teams and lose access. Their Auth account remains \u2014 if they log in again, they will be re-created as a plain Member.`)) return;
  ...
  document.getElementById("signOutBtn")?.addEventListener("click", () => signOut2());
  var showApp = () => {
